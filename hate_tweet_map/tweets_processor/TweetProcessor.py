...
class ProcessTweet:
        ...
        if self.image_captioning:
            ...
            if len(tweets_to_image_captioning) > 0:
                ...
                for tweet in tqdm(tweets_to_image_captioning,desc = "IMAGE CAPTIONING PHASE", leave=True):
                    ...
                    if has_media:
                        count = 0
                        for media in tweet['media_urls']:
                            temp_url = tweet['media_urls']['media_url_' + str(count)]
                            temp_type = tweet['media_types']['media_type_' + str(count)]
                            if temp_type == "photo":
                                id, result, tweet = self.__image_captioning(temp_url, tweet)
                                if id == 8:

                                    if 'image_to_text' in tweet:
                                        tweet['image_to_text']['image_captioning' + "_" + str(count)] = result
                                    else:
                                        tweet['image_to_text'] = {}
                                        tweet['image_to_text']['image_captioning' + "_" + str(count)] = result
    ...
    def __image_captioning(self, tweet_media_url: str, tweet: dict) -> Tuple[int, dict, dict]:
        image_captioning = pipeline("image-to-text", model="nlpconnect/vit-gpt2-image-captioning")
        try:
            result = image_captioning(tweet_media_url)
        except:
            #if the model is busy wait and retry
            time.sleep(0.01)
            return self.__image_captioning(tweet)
        return 8, result, tweet
