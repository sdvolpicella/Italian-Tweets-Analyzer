...
model_path_hate_speech_it = "IMSyPP/hate_speech_it"
model_path_roberta_classifier = "classla/xlm-roberta-base-multilingual-text-genre-classifier"
mode_path_image_captioning = "nlpconnect/vit-gpt2-image-captioning"
...
class ProcessTweet:
    ...
    def __init__(self, path_to_cnfg_file):
        ...
        self.sent_it = False
        self.hate_speech_it = False
        self.roberta_classifier = False
        self.image_captioning = False
        self.geo = False
        self.tag_me = False
    ...
    def __load_configuration(self):
        with open(self.cnfg_file_path, "r") as ymlfile:
            ...
            self.hate_speech_it = cfg['analyzes']['sentiment_analyze']['hate_speech_it']
            self.roberta_classifier = cfg['analyzes']['genre_classification']['roberta']
            self.image_captioning = cfg['analyzes']['image_to_text']['image_captioning']
    ...
    def start(self):
        ...
        if self.feel_it:
            ...
            if len(tweets_to_feel_it) > 0:
                ...
                for tweet in tqdm(tweets_to_feel_it, desc="FEEL-IT PHASE", leave=True):
                    ...
                    # analyze the tweet with feel-it module
                    id, result, tweet = self.__feel_it_analyze_sentiment(tweet_text, tweet)
                    # save the result
                    if id == 3:
                        # if a sentiment analysis are yet done, adds the result of fee-it to it
        ...
        # if the hate_speech_it analysis have to be done
        if self.hate_speech_it:
            # if must be analyzed all tweets in the db extract from it all tweets
            # in italian language
            if self.all_tweets:
                tweets_to_hate_speech_it = self.mongo_db.extract_all_tweets_to_hate_speech_it()
            # if not all tweets have to be analyzed, extract from the db only the tweets that have are not been
            # processed by spacy and that are in italian language
            else:
                tweets_to_hate_speech_it = self.mongo_db.extract_new_tweets_to_hate_speech_it()
            if len(tweets_to_hate_speech_it) > 0:
                self.log.info("HATE_SPEECH_IT PHASE: PRE-EXECUTION")
                for tweet in tqdm(tweets_to_hate_speech_it, desc="HATE_SPEECH_IT PHASE", leave=True):
                    # extract the text of the tweets
                    tweet_text = tweet['raw_text']
                    # analyze the tweet with hate_speech_it module
                    id, result, tweet = self.__hate_speech_it_analyze_sentiment(tweet_text, tweet)
                    if id == 6:
                        # if a sentiment analysis are yet done, adds the result of the hate_speech_it to it
                        if 'sentiment' in tweet:
                            tweet['sentiment']['hate_speech_it'] = result
                        # else create the field sentiment, add the field hate_speech_it and save the result of the analysis
                        else:
                            tweet['sentiment'] = {}
                            tweet['sentiment']['hate_speech_it'] = result
                    self.mongo_db.update_one(tweet)
            else:
                self.log.info("HATE_SPEECH_IT PHASE: NO TWEETS FOUND TO THE PROCESS")

        # if the roberta's genre classification have to be done
        if self.roberta_classifier:
            # classify all tweets in the database
            if self.all_tweets:
                tweets_to_classify = self.mongo_db.extract_all_tweets_to_classify_roberta()
            # else classify the tweets in the database that haven't been processed by spacy and
            # that are in italian language
            else:
                tweets_to_classify = self.mongo_db.extract_new_tweets_to_classify_roberta()
            if len(tweets_to_classify) > 0:
                self.log.info("ROBERTA'S CLASSIFICATION PHASE: PRE-EXECUTION")
                for tweet in tqdm(tweets_to_classify, desc="ROBERTA'S CLASSIFICATION PHASE", leave=True):
                    # extract the text of the tweets
                    tweet_text = tweet['raw_text']
                    # classify the tweets with roberta
                    id, result, tweet = self.__classification_with_roberta(tweet_text, tweet)
                    if id == 7:
                        #
                        if 'genre_classification' in tweet:
                            tweet['genre_classification']['roberta'] = result
                        #
                        else:
                            tweet['genre_classification'] = {}
                            tweet['genre_classification']['roberta'] = result
                    self.mongo_db.update_one(tweet)
            else:
                self.log.info("ROBERTA'S CLASSIFICATION PHASE: NO TWEETS FOUND TO THE PROCESS")

        if self.image_captioning:
            tweets_to_image_captioning = self.mongo_db.extract_tweets_to_image_captioning()
            if len(tweets_to_image_captioning) > 0:
                self.log.info("IMAGE CAPTIONING PHASE: PRE-EXECUTION")
                for tweet in tqdm(tweets_to_image_captioning, desc="IMAGE CAPTIONING PHASE", leave=True):
                    ...
                    if has_media:
                        ...
                        i = 0
                        urls = []
                        for media in tweet['media_types']:
                            if media == "photo":
                                urls.append(tweet['media_urls'][i])
                            i += 1
                        if (len(urls) > 0):
                            id, result, tweet = self.__image_captioning(urls, tweet)
                            if id == 8:
                                # the image captioning model give us a result in the format [0]['generated_text],
                                # i did the following for loop to remove that
                                captions = []
                                for r in result:
                                    captions.append(r[0]['generated_text'])

                                if 'image_to_text' in tweet:
                                    tweet['image_to_text']['image_captioning'] = captions
                                else:
                                    tweet['image_to_text'] = {}
                                    tweet['image_to_text']['image_captioning'] = captions
                        ...
                        self.log.info("IMAGE CAPTIONING PHASE: TWEET WITHOUT MEDIAS")
                    self.mongo_db.update_one(tweet)
            else:
                self.log.info("IMAGE CAPTIONING PHASE: NO TWEETS FOUND THAT CAN BE PROCESSED")
    ...
    def __hate_speech_it_analyze_sentiment(self, tweet_text: str, tweet: dict) -> Tuple[int, dict, dict]:
        classifier = pipeline("sentiment-analysis", model=model_path_hate_speech_it,
                              tokenizer=model_path_hate_speech_it)
        hold_list = [tweet_text]
        temp_hate_level = " "
        temp_score = " "
        try:
            # sentiment_analyses: list[str] = [classifier(hold_list)[0]]
            ...
            # regex that removes everything from string but the score
            temp_score = re.findall(r"[-+]?(?:\d*\.*\d+)", temp_string)
            temp_score = temp_score[0]
            temp_score = float(temp_score)


            sentiment_analyses: dict[str][float] = {'hate_speech_level': temp_hate_level, 'score': temp_score}

        except:
            # if the model is busy wait and retry
            time.sleep(0.01)
            return self.__hate_speech_it_analyze_sentiment(tweet)
        # return the process id (6), the result and the tweet
        return 6, sentiment_analyses, tweet

    def __classification_with_roberta(self, tweet_text: str, tweet: dict) -> Tuple[int, dict, dict]:
        classifier = pipeline("text-classification", model=model_path_roberta_classifier,
    ...
    def __image_captioning(self, urls: list, tweet: dict) -> Tuple[int, dict, dict]:
