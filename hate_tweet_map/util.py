...
def pre_process_tweets_response(tweet: dict, includes: dict) -> dict:
    ...
    ent = {}
    post = {'_id': tweet['id'], 'raw_text': tweet['text'], 'author_id': tweet['author_id']} #attach potrebbe essere utile per effettuare controlli sui media
    retweeted = False
    user_location = None
    ...
    #check if there is a media in the tweet
    has_media = False
    for d in tweet:
        if d == "attachments":
            has_media = True

    # if it has media, add media fields
    if has_media:
        count = 0
        post['attachments'] = tweet['attachments']
        post['media_urls'] = {}
        post['media_types'] = {}
        for mk in post['attachments']['media_keys']:

            for m in includes['media']:
                if m['media_key'] == mk:
                    post['media_types']['media_type' + "_" + str(count)] = m['type']
                    if m['type'] == "photo":
                        post['media_urls']['media_url' + "_" + str(count)] = m['url']
                        count = count + 1
                    elif m['type'] == "animated_gif":
                        post['media_urls']['media_url' + "_" + str(count)] = m['preview_image_url']
                        count = count + 1
                    elif m['type'] == "video":
                        post['media_urls']['media_url' + "_" + str(count)] = m['preview_image_url']
                        count = count + 1
